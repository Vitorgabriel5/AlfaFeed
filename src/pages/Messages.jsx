...
function Messages() {
      ...
      <div className="min-h-screen bg-white dark:bg-black p-6 flex items-center justify-center">
        <div className="p-8 text-center max-w-md">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-6">
            Siga alguém para começar uma conversa!
          </p>
          <Link
            to="/feed"
            className="inline-block bg-gradient-to-r from-orange-500 to-red-600 text-white font-semibold px-6 py-3 rounded-xl hover:opacity-90 transition"
          >
            Voltar ao Feed
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black p-6">
      <div className="max-w-7xl mx-auto bg-white dark:bg-black rounded-2xl overflow-hidden h-[calc(100vh-3rem)] flex border border-gray-200 dark:border-gray-800">

        {/* Lista de conversas */}
        <aside className="w-[30%] border-r border-gray-200 dark:border-gray-800">
          ...
          <div className="overflow-y-auto h-[calc(100%-4.5rem)]">
            {conversations.map((conv) => {
              const avatarUrl = getAvatarUrl(conv.profilePicture, conv.id);
              ...
              return (
                <button
                  ...
                  className={`w-full px-4 py-3 text-left border-b border-gray-200 dark:border-gray-800 flex items-center gap-3 transition ${
                ...
                >
                  <div className="relative flex-shrink-0">
                    {avatarUrl ? (
                      ...
                      <DefaultAvatar name={conv.username} size="lg" />
                    )}
                    {unreadMap[conv.id] > 0 && (
        ...
        {/* Área de mensagens */}
        <section className="w-[70%] flex flex-col bg-white dark:bg-black">
          {activeConversation && (
            <>
              <header className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center gap-3">
                {getAvatarUrl(activeConversation.profilePicture, activeConversation.id) ? (
                  <img
                    src={getAvatarUrl(activeConversation.profilePicture, activeConversation.id)}
                    alt={activeConversation.nome}
                    className="h-11 w-11 rounded-full object-cover"
                  />
                ) : (
                  <DefaultAvatar name={activeConversation.username} size="lg" />
                )}
                <div>
                  <Link
                    ...
                    <div key={msg.id} className={`flex ${fromMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-md rounded-2xl px-4 py-2 text-sm shadow ${
                        fromMe
                          ...
                          : 'bg-gray-100 dark:bg-gray-900 text-gray-700 dark:text-gray-200 rounded-bl-md'
                      }`}>
                        {msg.type === 'image' ? (
                          <img
              ...
              <div className="p-4 border-t border-gray-200 dark:border-gray-800 relative">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="h-10 w-10 rounded-full bg-gray-100 dark:bg-gray-900 text-lg hover:bg-gray-200 dark:hover:bg-gray-800 transition flex items-center justify-center"
                  >
                    😊
                  </button>
                  <button
                  ...
                  <input
                    ...
                    className="flex-1 border border-gray-300 dark:border-gray-800 bg-white dark:bg-black text-gray-900 dark:text-white placeholder-gray-400 rounded-xl px-4 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500"
                  />
                  <button
                    type="button"
                    onClick={handleSendText}
                    disabled={!messageInput.trim()}
                    className="bg-gradient-to-r from-orange-500 to-red-600 text-white font-bold py-2 px-5 rounded-xl hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Enviar
                  </button>
                </div>
